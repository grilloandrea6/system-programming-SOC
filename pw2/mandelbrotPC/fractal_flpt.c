...
//printf("calc_mandelbrot_point_soft\n");
  float x = cx;
  float y = cy;
  uint16_t n = 0;
  float xx, yy, two_xy;
  do {
    ...
    y = two_xy + cy;
    ++n;
if(xx+yy>maximum) maximum = xx+yy;
...
rgb565 iter_to_grayscale(uint16_t iter, uint16_t n_max) {
  ...
  res = ((brightness << 12) | ((brightness << 7) | brightness<<1));
...
rgb565 iter_to_colour(uint16_t iter, uint16_t n_max) {
  ...
  rgb565 res = ((r & 0x1f) << 11) | ((g & 0x1f) << 6) | ((b & 0x1f));
...
rgb565 iter_to_colour1(uint16_t iter, uint16_t n_max) {
  ...
  rgb565 res = ((r & 0xf) << 12) | ((g & 0xf) << 7) | ((b & 0xf)<<1);
