...
uint16_t calc_mandelbrot_point_soft(float cx, float cy, uint16_t n_max) {
  ...
  do {
    EDDAI = multMyFloat(floatToMyFloat(x),floatToMyFloat(x));
  
    xx = myFloatToFloat(EDDAI);
    EDDAI = multMyFloat(floatToMyFloat(y),floatToMyFloat(y));
    yy = myFloatToFloat(EDDAI);//y*y;
    
    EDDAI = multiplyByTwo(multMyFloat(floatToMyFloat(x),floatToMyFloat(y)));
    two_xy = myFloatToFloat(EDDAI);

    EDDAI = sumMyFloat(sumMyFloat(floatToMyFloat(xx),floatToMyFloat(cx)),changeSign(floatToMyFloat(yy)));
    x = myFloatToFloat(EDDAI);//xx - yy + cx;
    
    EDDAI = sumMyFloat(floatToMyFloat(two_xy),floatToMyFloat(cy));
    y = myFloatToFloat(EDDAI);//two_xy + cy;
    ++n;
    
    
    EDDAI = sumMyFloat(floatToMyFloat(xx),floatToMyFloat(yy));

    
    
  } while (lessThan(EDDAI,four) && (n < n_max));
  return n;
}
...
uint32_t sumMyFloat(uint32_t a, uint32_t b) {
	...
	if(man < 0) 
		...
		//or32Print(&vgaPrintChar, "PRODOTTO\n");
	
	// rescaling
	if(manRes >> 23 == 0) {
		while((manRes >> 23) == 0 && manRes != 0) {
				//or32Print(&vgaPrintChar, "shifting LEFT\n");
			manRes <<= 1;
			expRes--;
		}
	} else {
		while((manRes >> 24)  != 0) {
								//or32Print(&vgaPrintChar, "shifting RIGHT\n");
			manRes >>= 1;
			expRes++;
		}
	}
