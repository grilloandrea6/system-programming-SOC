...
uint16_t calc_mandelbrot_point_soft(myFloat cx, myFloat cy, uint16_t n_max) {
  ...
  do {
    xx = multMyFloat(x,x);
    yy = multMyFloat(y,y);
    two_xy = multiplyByTwo(multMyFloat(x,y));

    x = sumMyFloat(sumMyFloat(xx,cx),changeSign(yy));
    y = sumMyFloat(two_xy,cy);
    ++n;
  } while (lessThan(sumMyFloat(xx,yy),MYFLOAT_FOUR) && (n < n_max));
  return n;
}
...
void draw_fractal(rgb565 *fbuf, int width, int height,
  ...
  for (int k = 0; k < height; ++k) {
    ...
    for(int i = 0; i < width; ++i) {
      ...
      *(pixel++) = colour;
      cx = sumMyFloat(cx,delta);
    }
    cy = sumMyFloat(cy,delta);
  }
}
...
uint32_t multMyFloat(uint32_t a, uint32_t b)
{
	...
	if(manRes >> 23 == 0) {
		while((manRes >> 23) == 0) {
   or32Print(&vgaPrintChar, "shifting BBBBbbBBBBBBBBBa\n");
			manRes <<= 1;
			expRes--;
		}
	} else {
		while((manRes >> 24)  != 0) {
   //or32Print(&vgaPrintChar, "shifting AAAAAAAaa\n");
			manRes >>= 1;
			expRes++;
		}
	}
